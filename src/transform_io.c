...
/*
static apr_status_t io_pass_failure(ap_filter_t * filter, const char *msg, transform_notes * notes)
{
    ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, filter->r, "Transform IO: %s", msg);
...
static apr_status_t ex_apr_uri_resolve_relative(apr_pool_t * pool, apr_uri_t * base, apr_uri_t * uptr)
{
    if (uptr == NULL || base == NULL || !base->is_initialized || !uptr->is_initialized)
        return APR_EGENERAL;
...
static int transform_xmlio_input_read(void *context, char *buffer, int len)
{
    ...
    if (!(input_ctx->bb))
    {
        ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, input_ctx->f->r, "mod_transform: Input Brigade was NULL.");
        len = 0;
        return -1;
    }
    ...
    if (rv != APR_SUCCESS)
    {
        ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, input_ctx->f->r, "mod_transform: Unable to Flatten Brigade into xmlIO Buffer");
        return -1;
    }
    ...
    if (rv != APR_SUCCESS)
    {
        ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, input_ctx->f->r, "mod_transform: Brigade Partition Failed!");
        return -1;
    }
    ...
    if (rv != APR_SUCCESS)
    {
        ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, input_ctx->f->r, "mod_transform: Brigade Destroy Failed!");
        return -1;
    }
...
static xmlParserInputBufferPtr transform_input_from_subrequest(ap_filter_t *f, const char *URI, xmlCharEncoding enc)
{
    int rr_status;
    ...
    if (rr_status != OK)
    {
        ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, input_ctx->f->r, "mod_transform: Subrequest for '%s' failed with '%d'", URI, rr_status);
        ap_destroy_sub_req(input_ctx->rr);
        apr_pool_destroy(subpool);
        return __xmlParserInputBufferCreateFilename(find_relative_uri(f, URI), enc);
    }
    ...
    {
        ap_log_rerror(APLOG_MARK, APLOG_ERR, 0, input_ctx->f->r, "mod_transform: Failed to create ParserInputBuffer");
        ap_destroy_sub_req(input_ctx->rr);
        apr_pool_destroy(subpool);
        return __xmlParserInputBufferCreateFilename(find_relative_uri(f, URI), enc);
    }
...
xmlParserInputBufferPtr transform_get_input(const char *URI, xmlCharEncoding enc)
{
	// If we were passed a NULL URI then this is over already.
    if (URI == NULL)
        return NULL;

    // Retrieve the filter from the XML generic error context.
	ap_filter_t *f = (ap_filter_t *) xmlGenericErrorContext;

    // If our context got killed somehow we can do no more.
    if (f == NULL)
        return NULL;

    // Get the per directory configuration.
    dir_cfg* dconf = ap_get_module_config(f->r->per_dir_config, &transform_module);

    // Does this directory specify an Apache based filesystem for libxml?
    if (dconf->opts & USE_APACHE_FS)
    {
    	// It does so we need to check if this is a request for the XML catalog or not,
    	// these requests should be loaded directly.  If it is any other request then
    	// we need to load it using a sub-request.
    ...
    // We're not using an Apache based filesystem so just load the file directly.
    return __xmlParserInputBufferCreateFilename(find_relative_uri(f, URI), enc);
}
